...
import CrushRobot.modules.sql.users_sql as sql
from CrushRobot import (
    BOT_NAME,
    BOT_USERNAME,
    CERT_PATH,
    DONATION_LINK,
...
from CrushRobot.modules.helper_funcs.chat_status import is_user_admin
from CrushRobot.modules.helper_funcs.misc import paginate_modules
...
buttons = [
    ...
    [
        InlineKeyboardButton(text="❄ ᴀʙᴏᴜᴛ ❄", callback_data="crush_"),
...
ʜᴀᴩᴩʏ ᴛᴏ ʜᴇᴀʀ ᴛʜᴀᴛ ʏᴏᴜ ᴡᴀɴɴᴀ ᴅᴏɴᴀᴛᴇ.ʏᴏᴜ ᴄᴀɴ ᴅɪʀᴇᴄᴛʟʏ ᴄᴏɴᴛᴀᴄᴛ ᴍʏ [ᴅᴇᴠᴇʟᴏᴩᴇʀ](https://t.me/aadillllll) ғᴏʀ ᴅᴏɴᴀᴛɪɴɢ ᴏʀ ʏᴏᴜ ᴄᴀɴ ᴠɪsɪᴛ ᴍʏ [sᴜᴩᴩᴏʀᴛ ᴄʜᴀᴛ](https://t.me/sudogroup) ᴀɴᴅ ᴀsᴋ ᴛʜᴇʀᴇ ᴀʙᴏᴜᴛ ᴅᴏɴᴀᴛɪᴏɴ."""
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("CrushRobot.modules." + module_name)
...
@run_async
def Crush_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "crush_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            text=f"*ʜᴇʏ,*🥀\n  *ᴛʜɪs ɪs {BOT_NAME}*"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="sᴜᴩᴩᴏʀᴛ", callback_data="crush_support"
                        ),
                        InlineKeyboardButton(
                    ...
                    [
                        InlineKeyboardButton(text="◁", callback_data="crush_back"),
                    ],
                ]
            ),
        )
    elif query.data == "crush_support":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        ...
                        InlineKeyboardButton(
                            text="ɢɪᴛʜᴜʙ",
                            callback_data="https://github.com/Darkranger00",
                        ),
                    ],
                    [
                        InlineKeyboardButton(text="◁", callback_data="crush_"),
                    ],
                ]
            ),
        )
    elif query.data == "crush_back":
        first_name = update.effective_user.first_name
        query.message.edit_text(
            PM_START_TEXT.format(escape_markdown(first_name), BOT_NAME),
            reply_markup=InlineKeyboardMarkup(buttons),
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 5652782615 and DONATION_LINK:
            update.effective_message.reply_text(
                f"» ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴩᴇʀ ᴏғ {BOT_NAME} sᴏʀᴄᴇ ᴄᴏᴅᴇ ɪs [ᴄʀᴜꜱʜ](https://t.me/aadillllll)."
...
┗•❅────✧❅✦❅✧────❅•┛""",
    ...
    about_callback_handler = CallbackQueryHandler(
        Crush_about_callback, pattern=r"crush_"
    )
